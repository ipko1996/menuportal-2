/**
 * Generated by orval v7.13.0 🍺
 * Do not edit manually.
 * API Documentation
 * The API description
 * OpenAPI spec version: 1.0
 */
import type {
  DataTag,
  DefinedInitialDataOptions,
  DefinedUseQueryResult,
  MutationFunction,
  QueryClient,
  QueryFunction,
  QueryKey,
  UndefinedInitialDataOptions,
  UseMutationOptions,
  UseMutationResult,
  UseQueryOptions,
  UseQueryResult,
} from '@tanstack/react-query';
import { useMutation, useQuery } from '@tanstack/react-query';

import type { BodyType,ErrorType } from '../../../utils/axios-instance';
import { axiosInstance } from '../../../utils/axios-instance';
import type {
  CreateDishDto,
  DishPaginatedResponseDto,
  DishResponseDto,
  GetPaginatedDishesParams,
  SearchDishesByNameParams,
  UpdateDishDto,
} from '../../schemas';

type SecondParameter<T extends (...args: never) => unknown> = Parameters<T>[1];

/**
 * @summary Create a new dish
 */
export const createDish = (
  createDishDto: BodyType<CreateDishDto>,
  options?: SecondParameter<typeof axiosInstance>,
  signal?: AbortSignal
) => {
  return axiosInstance<DishResponseDto>(
    {
      url: `/dish`,
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      data: createDishDto,
      signal,
    },
    options
  );
};

export const getCreateDishMutationOptions = <
  TError = ErrorType<void | void>,
  TContext = unknown
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof createDish>>,
    TError,
    { data: BodyType<CreateDishDto> },
    TContext
  >;
  request?: SecondParameter<typeof axiosInstance>;
}): UseMutationOptions<
  Awaited<ReturnType<typeof createDish>>,
  TError,
  { data: BodyType<CreateDishDto> },
  TContext
> => {
  const mutationKey = ['createDish'];
  const { mutation: mutationOptions, request: requestOptions } = options
    ? options.mutation &&
      'mutationKey' in options.mutation &&
      options.mutation.mutationKey
      ? options
      : { ...options, mutation: { ...options.mutation, mutationKey } }
    : { mutation: { mutationKey }, request: undefined };

  const mutationFn: MutationFunction<
    Awaited<ReturnType<typeof createDish>>,
    { data: BodyType<CreateDishDto> }
  > = props => {
    const { data } = props ?? {};

    return createDish(data, requestOptions);
  };

  return { mutationFn, ...mutationOptions };
};

export type CreateDishMutationResult = NonNullable<
  Awaited<ReturnType<typeof createDish>>
>;
export type CreateDishMutationBody = BodyType<CreateDishDto>;
export type CreateDishMutationError = ErrorType<void | void>;

/**
 * @summary Create a new dish
 */
export const useCreateDish = <
  TError = ErrorType<void | void>,
  TContext = unknown
>(
  options?: {
    mutation?: UseMutationOptions<
      Awaited<ReturnType<typeof createDish>>,
      TError,
      { data: BodyType<CreateDishDto> },
      TContext
    >;
    request?: SecondParameter<typeof axiosInstance>;
  },
  queryClient?: QueryClient
): UseMutationResult<
  Awaited<ReturnType<typeof createDish>>,
  TError,
  { data: BodyType<CreateDishDto> },
  TContext
> => {
  const mutationOptions = getCreateDishMutationOptions(options);

  return useMutation(mutationOptions, queryClient);
};
/**
 * @summary Get paginated dishes with search
 */
export const getPaginatedDishes = (
  params?: GetPaginatedDishesParams,
  options?: SecondParameter<typeof axiosInstance>,
  signal?: AbortSignal
) => {
  return axiosInstance<DishPaginatedResponseDto>(
    { url: `/dish`, method: 'GET', params, signal },
    options
  );
};

export const getGetPaginatedDishesQueryKey = (
  params?: GetPaginatedDishesParams
) => {
  return [`/dish`, ...(params ? [params] : [])] as const;
};

export const getGetPaginatedDishesQueryOptions = <
  TData = Awaited<ReturnType<typeof getPaginatedDishes>>,
  TError = ErrorType<unknown>
>(
  params?: GetPaginatedDishesParams,
  options?: {
    query?: Partial<
      UseQueryOptions<
        Awaited<ReturnType<typeof getPaginatedDishes>>,
        TError,
        TData
      >
    >;
    request?: SecondParameter<typeof axiosInstance>;
  }
) => {
  const { query: queryOptions, request: requestOptions } = options ?? {};

  const queryKey =
    queryOptions?.queryKey ?? getGetPaginatedDishesQueryKey(params);

  const queryFn: QueryFunction<
    Awaited<ReturnType<typeof getPaginatedDishes>>
  > = ({ signal }) => getPaginatedDishes(params, requestOptions, signal);

  return { queryKey, queryFn, ...queryOptions } as UseQueryOptions<
    Awaited<ReturnType<typeof getPaginatedDishes>>,
    TError,
    TData
  > & { queryKey: DataTag<QueryKey, TData, TError> };
};

export type GetPaginatedDishesQueryResult = NonNullable<
  Awaited<ReturnType<typeof getPaginatedDishes>>
>;
export type GetPaginatedDishesQueryError = ErrorType<unknown>;

export function useGetPaginatedDishes<
  TData = Awaited<ReturnType<typeof getPaginatedDishes>>,
  TError = ErrorType<unknown>
>(
  params: undefined | GetPaginatedDishesParams,
  options: {
    query: Partial<
      UseQueryOptions<
        Awaited<ReturnType<typeof getPaginatedDishes>>,
        TError,
        TData
      >
    > &
      Pick<
        DefinedInitialDataOptions<
          Awaited<ReturnType<typeof getPaginatedDishes>>,
          TError,
          Awaited<ReturnType<typeof getPaginatedDishes>>
        >,
        'initialData'
      >;
    request?: SecondParameter<typeof axiosInstance>;
  },
  queryClient?: QueryClient
): DefinedUseQueryResult<TData, TError> & {
  queryKey: DataTag<QueryKey, TData, TError>;
};
export function useGetPaginatedDishes<
  TData = Awaited<ReturnType<typeof getPaginatedDishes>>,
  TError = ErrorType<unknown>
>(
  params?: GetPaginatedDishesParams,
  options?: {
    query?: Partial<
      UseQueryOptions<
        Awaited<ReturnType<typeof getPaginatedDishes>>,
        TError,
        TData
      >
    > &
      Pick<
        UndefinedInitialDataOptions<
          Awaited<ReturnType<typeof getPaginatedDishes>>,
          TError,
          Awaited<ReturnType<typeof getPaginatedDishes>>
        >,
        'initialData'
      >;
    request?: SecondParameter<typeof axiosInstance>;
  },
  queryClient?: QueryClient
): UseQueryResult<TData, TError> & {
  queryKey: DataTag<QueryKey, TData, TError>;
};
export function useGetPaginatedDishes<
  TData = Awaited<ReturnType<typeof getPaginatedDishes>>,
  TError = ErrorType<unknown>
>(
  params?: GetPaginatedDishesParams,
  options?: {
    query?: Partial<
      UseQueryOptions<
        Awaited<ReturnType<typeof getPaginatedDishes>>,
        TError,
        TData
      >
    >;
    request?: SecondParameter<typeof axiosInstance>;
  },
  queryClient?: QueryClient
): UseQueryResult<TData, TError> & {
  queryKey: DataTag<QueryKey, TData, TError>;
};
/**
 * @summary Get paginated dishes with search
 */

export function useGetPaginatedDishes<
  TData = Awaited<ReturnType<typeof getPaginatedDishes>>,
  TError = ErrorType<unknown>
>(
  params?: GetPaginatedDishesParams,
  options?: {
    query?: Partial<
      UseQueryOptions<
        Awaited<ReturnType<typeof getPaginatedDishes>>,
        TError,
        TData
      >
    >;
    request?: SecondParameter<typeof axiosInstance>;
  },
  queryClient?: QueryClient
): UseQueryResult<TData, TError> & {
  queryKey: DataTag<QueryKey, TData, TError>;
} {
  const queryOptions = getGetPaginatedDishesQueryOptions(params, options);

  const query = useQuery(queryOptions, queryClient) as UseQueryResult<
    TData,
    TError
  > & { queryKey: DataTag<QueryKey, TData, TError> };

  query.queryKey = queryOptions.queryKey;

  return query;
}

/**
 * @summary Search for dishes by name (fuzzy search)
 */
export const searchDishesByName = (
  params: SearchDishesByNameParams,
  options?: SecondParameter<typeof axiosInstance>,
  signal?: AbortSignal
) => {
  return axiosInstance<DishResponseDto[]>(
    { url: `/dish/search`, method: 'GET', params, signal },
    options
  );
};

export const getSearchDishesByNameQueryKey = (
  params?: SearchDishesByNameParams
) => {
  return [`/dish/search`, ...(params ? [params] : [])] as const;
};

export const getSearchDishesByNameQueryOptions = <
  TData = Awaited<ReturnType<typeof searchDishesByName>>,
  TError = ErrorType<void>
>(
  params: SearchDishesByNameParams,
  options?: {
    query?: Partial<
      UseQueryOptions<
        Awaited<ReturnType<typeof searchDishesByName>>,
        TError,
        TData
      >
    >;
    request?: SecondParameter<typeof axiosInstance>;
  }
) => {
  const { query: queryOptions, request: requestOptions } = options ?? {};

  const queryKey =
    queryOptions?.queryKey ?? getSearchDishesByNameQueryKey(params);

  const queryFn: QueryFunction<
    Awaited<ReturnType<typeof searchDishesByName>>
  > = ({ signal }) => searchDishesByName(params, requestOptions, signal);

  return { queryKey, queryFn, ...queryOptions } as UseQueryOptions<
    Awaited<ReturnType<typeof searchDishesByName>>,
    TError,
    TData
  > & { queryKey: DataTag<QueryKey, TData, TError> };
};

export type SearchDishesByNameQueryResult = NonNullable<
  Awaited<ReturnType<typeof searchDishesByName>>
>;
export type SearchDishesByNameQueryError = ErrorType<void>;

export function useSearchDishesByName<
  TData = Awaited<ReturnType<typeof searchDishesByName>>,
  TError = ErrorType<void>
>(
  params: SearchDishesByNameParams,
  options: {
    query: Partial<
      UseQueryOptions<
        Awaited<ReturnType<typeof searchDishesByName>>,
        TError,
        TData
      >
    > &
      Pick<
        DefinedInitialDataOptions<
          Awaited<ReturnType<typeof searchDishesByName>>,
          TError,
          Awaited<ReturnType<typeof searchDishesByName>>
        >,
        'initialData'
      >;
    request?: SecondParameter<typeof axiosInstance>;
  },
  queryClient?: QueryClient
): DefinedUseQueryResult<TData, TError> & {
  queryKey: DataTag<QueryKey, TData, TError>;
};
export function useSearchDishesByName<
  TData = Awaited<ReturnType<typeof searchDishesByName>>,
  TError = ErrorType<void>
>(
  params: SearchDishesByNameParams,
  options?: {
    query?: Partial<
      UseQueryOptions<
        Awaited<ReturnType<typeof searchDishesByName>>,
        TError,
        TData
      >
    > &
      Pick<
        UndefinedInitialDataOptions<
          Awaited<ReturnType<typeof searchDishesByName>>,
          TError,
          Awaited<ReturnType<typeof searchDishesByName>>
        >,
        'initialData'
      >;
    request?: SecondParameter<typeof axiosInstance>;
  },
  queryClient?: QueryClient
): UseQueryResult<TData, TError> & {
  queryKey: DataTag<QueryKey, TData, TError>;
};
export function useSearchDishesByName<
  TData = Awaited<ReturnType<typeof searchDishesByName>>,
  TError = ErrorType<void>
>(
  params: SearchDishesByNameParams,
  options?: {
    query?: Partial<
      UseQueryOptions<
        Awaited<ReturnType<typeof searchDishesByName>>,
        TError,
        TData
      >
    >;
    request?: SecondParameter<typeof axiosInstance>;
  },
  queryClient?: QueryClient
): UseQueryResult<TData, TError> & {
  queryKey: DataTag<QueryKey, TData, TError>;
};
/**
 * @summary Search for dishes by name (fuzzy search)
 */

export function useSearchDishesByName<
  TData = Awaited<ReturnType<typeof searchDishesByName>>,
  TError = ErrorType<void>
>(
  params: SearchDishesByNameParams,
  options?: {
    query?: Partial<
      UseQueryOptions<
        Awaited<ReturnType<typeof searchDishesByName>>,
        TError,
        TData
      >
    >;
    request?: SecondParameter<typeof axiosInstance>;
  },
  queryClient?: QueryClient
): UseQueryResult<TData, TError> & {
  queryKey: DataTag<QueryKey, TData, TError>;
} {
  const queryOptions = getSearchDishesByNameQueryOptions(params, options);

  const query = useQuery(queryOptions, queryClient) as UseQueryResult<
    TData,
    TError
  > & { queryKey: DataTag<QueryKey, TData, TError> };

  query.queryKey = queryOptions.queryKey;

  return query;
}

/**
 * @summary Get a dish by ID
 */
export const getDishById = (
  id: number,
  options?: SecondParameter<typeof axiosInstance>,
  signal?: AbortSignal
) => {
  return axiosInstance<DishResponseDto>(
    { url: `/dish/${id}`, method: 'GET', signal },
    options
  );
};

export const getGetDishByIdQueryKey = (id?: number) => {
  return [`/dish/${id}`] as const;
};

export const getGetDishByIdQueryOptions = <
  TData = Awaited<ReturnType<typeof getDishById>>,
  TError = ErrorType<void>
>(
  id: number,
  options?: {
    query?: Partial<
      UseQueryOptions<Awaited<ReturnType<typeof getDishById>>, TError, TData>
    >;
    request?: SecondParameter<typeof axiosInstance>;
  }
) => {
  const { query: queryOptions, request: requestOptions } = options ?? {};

  const queryKey = queryOptions?.queryKey ?? getGetDishByIdQueryKey(id);

  const queryFn: QueryFunction<Awaited<ReturnType<typeof getDishById>>> = ({
    signal,
  }) => getDishById(id, requestOptions, signal);

  return {
    queryKey,
    queryFn,
    enabled: !!id,
    ...queryOptions,
  } as UseQueryOptions<
    Awaited<ReturnType<typeof getDishById>>,
    TError,
    TData
  > & { queryKey: DataTag<QueryKey, TData, TError> };
};

export type GetDishByIdQueryResult = NonNullable<
  Awaited<ReturnType<typeof getDishById>>
>;
export type GetDishByIdQueryError = ErrorType<void>;

export function useGetDishById<
  TData = Awaited<ReturnType<typeof getDishById>>,
  TError = ErrorType<void>
>(
  id: number,
  options: {
    query: Partial<
      UseQueryOptions<Awaited<ReturnType<typeof getDishById>>, TError, TData>
    > &
      Pick<
        DefinedInitialDataOptions<
          Awaited<ReturnType<typeof getDishById>>,
          TError,
          Awaited<ReturnType<typeof getDishById>>
        >,
        'initialData'
      >;
    request?: SecondParameter<typeof axiosInstance>;
  },
  queryClient?: QueryClient
): DefinedUseQueryResult<TData, TError> & {
  queryKey: DataTag<QueryKey, TData, TError>;
};
export function useGetDishById<
  TData = Awaited<ReturnType<typeof getDishById>>,
  TError = ErrorType<void>
>(
  id: number,
  options?: {
    query?: Partial<
      UseQueryOptions<Awaited<ReturnType<typeof getDishById>>, TError, TData>
    > &
      Pick<
        UndefinedInitialDataOptions<
          Awaited<ReturnType<typeof getDishById>>,
          TError,
          Awaited<ReturnType<typeof getDishById>>
        >,
        'initialData'
      >;
    request?: SecondParameter<typeof axiosInstance>;
  },
  queryClient?: QueryClient
): UseQueryResult<TData, TError> & {
  queryKey: DataTag<QueryKey, TData, TError>;
};
export function useGetDishById<
  TData = Awaited<ReturnType<typeof getDishById>>,
  TError = ErrorType<void>
>(
  id: number,
  options?: {
    query?: Partial<
      UseQueryOptions<Awaited<ReturnType<typeof getDishById>>, TError, TData>
    >;
    request?: SecondParameter<typeof axiosInstance>;
  },
  queryClient?: QueryClient
): UseQueryResult<TData, TError> & {
  queryKey: DataTag<QueryKey, TData, TError>;
};
/**
 * @summary Get a dish by ID
 */

export function useGetDishById<
  TData = Awaited<ReturnType<typeof getDishById>>,
  TError = ErrorType<void>
>(
  id: number,
  options?: {
    query?: Partial<
      UseQueryOptions<Awaited<ReturnType<typeof getDishById>>, TError, TData>
    >;
    request?: SecondParameter<typeof axiosInstance>;
  },
  queryClient?: QueryClient
): UseQueryResult<TData, TError> & {
  queryKey: DataTag<QueryKey, TData, TError>;
} {
  const queryOptions = getGetDishByIdQueryOptions(id, options);

  const query = useQuery(queryOptions, queryClient) as UseQueryResult<
    TData,
    TError
  > & { queryKey: DataTag<QueryKey, TData, TError> };

  query.queryKey = queryOptions.queryKey;

  return query;
}

/**
 * @summary Update a dish by ID
 */
export const updateDishById = (
  id: number,
  updateDishDto: BodyType<UpdateDishDto>,
  options?: SecondParameter<typeof axiosInstance>
) => {
  return axiosInstance<DishResponseDto>(
    {
      url: `/dish/${id}`,
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      data: updateDishDto,
    },
    options
  );
};

export const getUpdateDishByIdMutationOptions = <
  TError = ErrorType<void | void>,
  TContext = unknown
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof updateDishById>>,
    TError,
    { id: number; data: BodyType<UpdateDishDto> },
    TContext
  >;
  request?: SecondParameter<typeof axiosInstance>;
}): UseMutationOptions<
  Awaited<ReturnType<typeof updateDishById>>,
  TError,
  { id: number; data: BodyType<UpdateDishDto> },
  TContext
> => {
  const mutationKey = ['updateDishById'];
  const { mutation: mutationOptions, request: requestOptions } = options
    ? options.mutation &&
      'mutationKey' in options.mutation &&
      options.mutation.mutationKey
      ? options
      : { ...options, mutation: { ...options.mutation, mutationKey } }
    : { mutation: { mutationKey }, request: undefined };

  const mutationFn: MutationFunction<
    Awaited<ReturnType<typeof updateDishById>>,
    { id: number; data: BodyType<UpdateDishDto> }
  > = props => {
    const { id, data } = props ?? {};

    return updateDishById(id, data, requestOptions);
  };

  return { mutationFn, ...mutationOptions };
};

export type UpdateDishByIdMutationResult = NonNullable<
  Awaited<ReturnType<typeof updateDishById>>
>;
export type UpdateDishByIdMutationBody = BodyType<UpdateDishDto>;
export type UpdateDishByIdMutationError = ErrorType<void | void>;

/**
 * @summary Update a dish by ID
 */
export const useUpdateDishById = <
  TError = ErrorType<void | void>,
  TContext = unknown
>(
  options?: {
    mutation?: UseMutationOptions<
      Awaited<ReturnType<typeof updateDishById>>,
      TError,
      { id: number; data: BodyType<UpdateDishDto> },
      TContext
    >;
    request?: SecondParameter<typeof axiosInstance>;
  },
  queryClient?: QueryClient
): UseMutationResult<
  Awaited<ReturnType<typeof updateDishById>>,
  TError,
  { id: number; data: BodyType<UpdateDishDto> },
  TContext
> => {
  const mutationOptions = getUpdateDishByIdMutationOptions(options);

  return useMutation(mutationOptions, queryClient);
};
/**
 * @summary Delete a dish by ID
 */
export const deleteDishById = (
  id: number,
  options?: SecondParameter<typeof axiosInstance>
) => {
  return axiosInstance<void>({ url: `/dish/${id}`, method: 'DELETE' }, options);
};

export const getDeleteDishByIdMutationOptions = <
  TError = ErrorType<void>,
  TContext = unknown
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof deleteDishById>>,
    TError,
    { id: number },
    TContext
  >;
  request?: SecondParameter<typeof axiosInstance>;
}): UseMutationOptions<
  Awaited<ReturnType<typeof deleteDishById>>,
  TError,
  { id: number },
  TContext
> => {
  const mutationKey = ['deleteDishById'];
  const { mutation: mutationOptions, request: requestOptions } = options
    ? options.mutation &&
      'mutationKey' in options.mutation &&
      options.mutation.mutationKey
      ? options
      : { ...options, mutation: { ...options.mutation, mutationKey } }
    : { mutation: { mutationKey }, request: undefined };

  const mutationFn: MutationFunction<
    Awaited<ReturnType<typeof deleteDishById>>,
    { id: number }
  > = props => {
    const { id } = props ?? {};

    return deleteDishById(id, requestOptions);
  };

  return { mutationFn, ...mutationOptions };
};

export type DeleteDishByIdMutationResult = NonNullable<
  Awaited<ReturnType<typeof deleteDishById>>
>;

export type DeleteDishByIdMutationError = ErrorType<void>;

/**
 * @summary Delete a dish by ID
 */
export const useDeleteDishById = <TError = ErrorType<void>, TContext = unknown>(
  options?: {
    mutation?: UseMutationOptions<
      Awaited<ReturnType<typeof deleteDishById>>,
      TError,
      { id: number },
      TContext
    >;
    request?: SecondParameter<typeof axiosInstance>;
  },
  queryClient?: QueryClient
): UseMutationResult<
  Awaited<ReturnType<typeof deleteDishById>>,
  TError,
  { id: number },
  TContext
> => {
  const mutationOptions = getDeleteDishByIdMutationOptions(options);

  return useMutation(mutationOptions, queryClient);
};
